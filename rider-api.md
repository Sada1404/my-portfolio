...
| **Maps & routing** | **@turf/turf** (geospatial), **Google Distance Matrix API**, **OSRM** (fallback), Haversine fallback |
| **File uploads** | **Multer** |
...
| **State** | **React Context** (AuthContext, OngoingOrderContext); **Redux Toolkit** + **react-redux** in dependencies (available for future use) |
